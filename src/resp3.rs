...
fn parse_pairs(
    ...
    let mut cursor = after_crlf;
    let mut pairs = Vec::with_capacity(bounded_capacity(buf, after_crlf, count, 2));
    if count > 0 {
        let child_depth = depth.checked_sub(1).ok_or(ParseError::DepthExceeded)?;
        for _ in 0..count {
...
/// 11-byte header reserve over a gigabyte. Clamping it to what the remaining
/// bytes could actually hold, at `MIN_FRAME_SIZE` per frame, keeps the
/// reservation proportional to the input.
///
